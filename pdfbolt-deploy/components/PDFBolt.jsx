...
const T = {
  en: {
    ...
    faq: {
      ...
      qs: [
        ...
        ["How do I manage my subscription?", "Click 'Manage subscription' in the top bar. You'll be taken to a secure Stripe portal where you can cancel, change your card, or download invoices — all self-service, no contact needed."],
  ...
  fr: {
    ...
    faq: {
      ...
      qs: [
        ["Mes fichiers sont-ils en sécurité ?", "PDFBolt traite tout dans votre navigateur. Vos PDF ne sont jamais envoyés sur un serveur. Nous ne voyons, ne stockons, ni ne touchons vos documents."],
        ["Ça fonctionne hors-ligne ?", "Oui. Une fois la page chargée, vous pouvez utiliser PDFBolt sans connexion."],
        ["Quelle est la limite gratuite ?", "3 opérations par jour avec les outils de base. Gratuit pour toujours, aucune inscription requise."],
        ["Faut-il un compte ?", "Non. Les outils gratuits fonctionnent sans compte. En passant à Pro, votre email de paiement est votre identité — pas de mot de passe. Un cookie sécurisé vous garde connecté automatiquement."],
        ["Comment gérer mon abonnement ?", "Cliquez sur 'Gérer abonnement' dans la barre en haut. Vous serez redirigé vers le portail sécurisé Stripe où vous pouvez annuler, changer de carte, ou télécharger vos factures — en toute autonomie."],
        ["Puis-je annuler le Pro ?", "Oui, à tout moment depuis le portail Stripe. Sans pénalité, sans question. Vous gardez l'accès jusqu'à la fin de la période payée."],
...
function Work({tool,usage,setUsage,go,isPro}){
  ...
  const run=async()=>{if(lim||proc)return;setProc(true);setProg(0);setResult(null);setErr(null);try{const p=v=>setProg(v);let r;switch(tool.id){case"merge":r=await doMerge(files,p);r.label=t.work.results.merged(r._n);break;case"compress":r=await doCompress(files,p);r.label=t.work.results.compressed(r._pct,r._from,r._to);break;case"to-images":r=await doImages(files,p);r.label=t.work.results.images(r._n);break;case"split":r=await doSplit(files,p);r.label=t.work.results.split(r._n);break;case"watermark":r=await doWatermark(files,p,wmText);r.label=t.work.results.watermark(r._text,r._n);break;case"rotate":r=await doRotate(files,p,rotDeg);r.label=t.work.results.rotate(r._n,r._deg);break;case"protect":r=await doProtect(files,p,pw);r.label=t.work.results.protect(r._pw);break;case"ocr":r=await doOCR(files,p);r.label=r._empty?t.work.results.noText:t.work.results.ocr(r._n);break}setResult(r);if(!isPro)setUsage(bumpU())}catch(e){setErr(e.message||"Error")}setProc(false)};
...
// ═══ STRIPE CHECKOUT + PRO AUTH ═══
async function goCheckout(plan = "monthly") {
  try {
    ...
    else alert(data.error || "Error. Retry.");
  } catch { alert("Connection error. Retry."); }
}
...
async function activateSession(sessionId) {
  try {
    const res = await fetch(`/api/activate?session_id=${sessionId}`);
    const data = await res.json();
    return data.pro === true;
  } catch { return false; }
}
...
export default function App(){
  ...
  // Check Pro status on mount + activate if returning from checkout
  useEffect(()=>{
    (async()=>{
      const params=new URLSearchParams(window.location.search);
      const sid=params.get("session_id");
      if(sid){
        const ok=await activateSession(sid);
        setIsPro(ok);
        // Clean URL
        window.history.replaceState({},"","/");
      } else {
        const ok=await checkProStatus();
        setIsPro(ok);
      }
      setProLoading(false);
    })();
